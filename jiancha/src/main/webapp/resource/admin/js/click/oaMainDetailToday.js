$(function(){
	oaMainDeatilToday.service.init();
});


var oaMainDeatilToday = {
		service : {
			init : function(){
				oaMainDeatilToday.service.dealClick();
				oaMainDeatilToday.controller.getPage(1);
			},
			dealClick : function(){
			    $(".data-table").on("click",".pageNumber", function(){
			    	var pageNo = $(this).attr("pageNo");
			    	oaMainDeatilToday.controller.getPage(pageNo);
			    });
...
//			    	}
		...
		controller : {
			getPage : function(pageNo){
				...
				$.ajax({
					url:contextPath+'/click/index/todayDetailTable',
					type:'POST',
					dataType : 'html',	

...
var oaMain = {
	dao : {
			createSevenCenterHtml : function(arr){
				...
				for(var data in arr){
					...
					html = html + '<div class="totalListWarp"><p class="totalListLabel fl">'+arr[data].modeName+'</p>';
	...
	service : {
		init : function(){
			oaMain.service.firstThreeCharts();
			oaMain.service.fourthCharts();
			oaMain.service.dealClick();
			oaMain.service.initPage();
			//oaMain.service.setForbidden();
			oaMain.service.setBlack();
			oaMain.service.setWhite();
		},
		initPage : function(){
			...
			oaMain.controller.getList(pageNo,"other");
		},
		firstThreeCharts : function(){
			oaMain.controller.getFirstThreeCharts();
		},
		fourthCharts : function(){
			oaMain.controller.getFourthCharts();
		},
		dealFirstThreeCharts : function(attach){
			...
			oaMain.service.dealChartOne(todayErrorCount, todayTotalCount);
			oaMain.service.dealChartTwo(sevendDayErrorCount, sevendDayTotalCount);
			oaMain.service.dealChartThree(centerArray);
		...
		dealChartThree : function(arr){
			...
			var html = oaMain.dao.createSevenCenterHtml(arr);
		...
		dealClick : function(){
			$(".addDateSelectWarp li").click(function(){
				...
				oaMain.controller.getList(1,"other");
			});
			...
			$(".data-table").on("click",".fileDetail", function(){
				...
				var dayType = $("#dayType").val();
				if(dayType == "DAY"){
					//window.open(contextPath+"/click/index/todayDetail?userName="+userName+"&pageNo="+pageNo+"&searchName="+searchName+"&state="+state+"&userType="+userType+"&dayType="+dayType);
					window.location.href=contextPath+"/click/index/todayDetail?userName="+userName+"&pageNo="+pageNo+"&search="+searchName+"&state="+state+"&userType="+userType+"&dayType="+dayType;
				}else if(dayType == "WEEK"){
					//window.open(contextPath+"/click/index/detail?userName="+userName+"&pageNo="+pageNo+"&searchName="+searchName+"&state="+state+"&userType="+userType+"&dayType="+dayType);
					window.location.href=contextPath+"/click/index/detail?userName="+userName+"&pageNo="+pageNo+"&search="+searchName+"&state="+state+"&userType="+userType+"&dayType="+dayType;
				}
			});
			
		    $(".data-table").on("click",".pageNumber", function(){
		    	var pageNo = $(this).attr("pageNo");
				oaMain.controller.getList(pageNo,"page");
		    });
...
//		    	}
		...
		setForbidden : function(){
			...
			$(".saveBtnSty").click(function(){
				...
				$(".cancelSty").click();
				oaMain.controller.forbidden(userName,forbiddenTime,forbiddenReason);
			});
		},
		setBlack : function(){
			$(".data-table").on("click",".operatorBlack", function(){
			...
			//	if(confirm("确定要对将此人加入黑名单吗？")){
					$(this).next().click();
					oaMain.controller.setUserType(userName,"black",addReason);
				//}
//				var flag = oaMain.service.validateBlackAndWhite(addReason);
//				if(flag){
//					$(this).next().click();
//					oaMain.controller.setUserType(userName,"black",addReason);
//				}
		...
		setWhite : function(){
			...
			$("#saveBtnWhite").click(function(){
				...
				//if(confirm("确定要对将此人加入白名单吗？")){
					$(this).next().click();
					oaMain.controller.setUserType(userName,"white",addReason);
			//	}
//				var flag = oaMain.service.validateBlackAndWhite(addReason);
//				if(flag){
//					$(this).next().click();
...
//				}
		...
		refreshPage : function(){
			oaMain.service.firstThreeCharts();//刷新前3个图表
			oaMain.service.fourthCharts();//刷新第四个图表
			var pageNo = $(".page-number .active").text();
			oaMain.controller.getList(pageNo,"page");//刷新列表
		},
	...
	controller : {
		getFirstThreeCharts : function(){
			$.ajax({
				...
				success:function(data){
					if(data.code == 1){
						oaMain.service.dealFirstThreeCharts(data.attach);
					}
				},
				error:function(){
					//alert("操作失败");
		...
		getFourthCharts : function(){
			$.ajax({
				...
				success:function(data){
					if(data.code == 1){
						oaMain.service.dealChartsFour(data.attach);
					}
				},
				error:function(){
					//alert("操作失败");
		...
		getList : function(pageNo,type){
			...
			var dayType = $("#dayType").val();
			$.ajax({
				url:contextPath+'/click/index/table',
				type:'POST',
				dataType : 'html',	
		...
		setUserType : function(userName,userType,addReason){
			$.ajax({
				...
				type:'POST',
				dataType : 'json',	
				data:{"userName":userName,"userType":userType,"limitType":"click","addReason":addReason},
				success:function(data){
					if(data.code == 1){
						layer.msg("操作成功");
						oaMain.service.refreshPage();
					}
				},
				error:function(){
					//alert("操作失败");

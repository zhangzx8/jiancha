/**
 * Created by hanpan on 2016/12/9.
 */
$(function () {
    //绘制大环形图
    ...
    //查看文件下载今日详情录详情操作detail
    var operateClass=$(".fileDetail");//当前内容行操作者
    var todayreturnBtnId=$("#fileDownTodayReturn");//当前内容层返回按钮btn
    var todaydivTypeId=$("#fileDownToday");//需要显示的当前内容层


    var weekreturnBtnId=$("#fileDowWeekReturn");
    var weekdivTypeId=$("#fileDowWeek");
    //var todayDetail=$("#fileDownToday");
    loaddetialDate(1,todaydivTypeId,operateClass);

    //文件下载今日详情信息返回操作

     //切换div显示状况
    //今日详情数据
     divShowState(todayreturnBtnId,todaydivTypeId);
    //一周详情数据
     divShowState(weekreturnBtnId,weekdivTypeId);
});

/****
 * 作用：表格查看详情操作
 * 传参：recordId（该行记录的标识，可以自行根据后台需要的内容传参此处只是模拟）
 * conterDiv (要显示那个内容层的标识)
 * operateClass(执行当前操作的标识)
 * 作者：韩盼
 * 日期：2016-12-09
 ...
 * ***/
function loaddetialDate(recordId,conterDiv,operateClass){
 ...
 * returnId--->当前返回按钮的id
 * divTypeId--->当前内容层的id
 * action:控制当前操作要显示的内容层
 * **/
function divShowState(returnId,divTypeId){

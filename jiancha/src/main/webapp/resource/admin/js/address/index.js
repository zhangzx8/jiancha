...
var oaAddress = {
	...
	service : {
		init : function(){
			oaAddress.service.firstThreeCharts();
			oaAddress.service.fourthCharts();
			oaAddress.service.dealClick();
			oaAddress.service.initPage();
			oaAddress.service.setForbidden();
			oaAddress.service.setBlack();
			oaAddress.service.setWhite();
		},
		initPage : function(){
			...
			oaAddress.controller.getList(pageNo,"other");
		},
		firstThreeCharts : function(){
			oaAddress.controller.getFirstThreeCharts();
		},
		fourthCharts : function(){
			oaAddress.controller.getFourthCharts();
		},
		dealFirstThreeCharts : function(attach){
			...
			oaAddress.service.dealChartOne(todayErrorCount, todayTotalCount);
			oaAddress.service.dealChartTwo(sevendDayErrorCount, sevendDayTotalCount);
			oaAddress.service.dealChartThree(centerArray);
		...
		dealChartThree : function(arr){
			...
			var html = oaAddress.dao.createSevenCenterHtml(arr);
		...
		dealClick : function(){
			$(".addDateSelectWarp li").click(function(){
				...
				oaAddress.controller.getList(1,"other");
			});
			$(".toolbtn").click(function(){
				oaAddress.controller.getList(1,"other");
			});
			$("#tableList").on("click",".address_searchDetial", function(){
				...
				var dayType = $("#dayType").val();
				if(dayType == "DAY"){
					//window.open(contextPath+"/address/index/todayDetail?userName="+userName);
					window.location.href=contextPath+"/address/index/todayDetail?userName="+userName+"&pageNo="+pageNo+"&search="+searchName+"&state="+state+"&userType="+userType+"&dayType="+dayType;
				}else if(dayType == "WEEK"){
					//window.open(contextPath+"/address/index/detail?userName="+userName);
					window.location.href=contextPath+"/address/index/detail?userName="+userName+"&pageNo="+pageNo+"&search="+searchName+"&state="+state+"&userType="+userType+"&dayType="+dayType;
				}
			});
...
//				$("#userRestrict").modal();
//			});
//			$("#tableList").on("click","#operatorBlack", function(){
//				$("#addBlackList").modal();
//			});
//			$("#tableList").on("click","#operatorWhite", function(){
//				$("#addWhiteList").modal();
//			});
			
		    $(".data-table").on("click",".pageNumber", function(){
		    	var pageNo = $(this).attr("pageNo");
				oaAddress.controller.getList(pageNo,"page");
		    });
		    ...
		    $("#forbiddenReason").on("input blur",function(){
		    	var forbiddenReason = $("#forbiddenReason").val();
//		    	if(forbiddenReason == ""){
//		    		$(this).next().text("限制原因不能为空").show();
//		    	}else if(forbiddenReason.length < 5 || forbiddenReason.length > 30){
//		    		$(this).next().text("请输入5-30个字符").show();
//		    	}else if(/^\d+$/.test(forbiddenReason)){
//		    		$(this).next().text("不能输入纯数字").show();
//		    	}else if(/[#$%&\^*￥@]+/.test(forbiddenReason)){
//		    		$(this).next().text("不能包含特殊字符").show();
//		    	}else if(!/[\u4e00-\u9fa5]+/.test(forbiddenReason)){
//		    		$(this).next().text("必须包含中文").show();
//		    	}else{
//		    		$(this).next().hide();
//		    	}
		    	if(forbiddenReason.length < 5 || forbiddenReason.length > 30){
		    		$(this).next().show();
		    	}else{
		    		$(this).next().hide();
		    	}
...
//		    	}
		...
		setForbidden : function(){
			...
			$("#saveBtnForbidden").click(function(){
				...
				//if(confirm("确定要对此用户进行限制吗")){
					var userName = $(this).prev().val();
					var forbiddenTime = $("#forbiddenTime").val();
					$(this).next().click();
					oaAddress.controller.forbidden(userName,forbiddenTime,forbiddenReason);
			//	}
//				var flag = oaAddress.service.validateBlackAndWhite(forbiddenReason);
//				if(flag){
//					if(confirm("确定要对此用户进行限制吗")){
//						var userName = $(this).prev().val();
//						var forbiddenTime = $("#forbiddenTime").val();
...
//				}
		...
		setBlack : function(){
			...
			//	if(confirm("确定要对将此人加入黑名单吗？")){
					$(this).next().click();
					oaAddress.controller.setUserType(userName,"black",addReason);
			//	}
//				var flag = oaAddress.service.validateBlackAndWhite(addReason);
//				if(flag){
//					$(this).next().click();
//					oaAddress.controller.setUserType(userName,"black",addReason);
//				}
		...
		setWhite : function(){
			...
			//	if(confirm("确定要对将此人加入白名单吗？")){
					$(this).next().click();
					oaAddress.controller.setUserType(userName,"white",addReason);
			//	}
			});
		},
		refreshPage : function(){
			oaAddress.service.firstThreeCharts();//刷新前3个图表
			oaAddress.service.fourthCharts();//刷新第四个图表
			var pageNo = $(".page-number .active").text();
			oaAddress.controller.getList(pageNo,"page");//刷新列表
		},
	...
	controller : {
		getFirstThreeCharts : function(){
			$.ajax({
				url:contextPath+'/address/ajax/firstThreeCharts',
				type:'POST',
				dataType : 'json',	
				data:{},
				success:function(data){
					if(data.code == 1){
						oaAddress.service.dealFirstThreeCharts(data.attach);
					}
				},
				error:function(){
					//alert("操作失败");
		...
		getFourthCharts : function(){
			$.ajax({
				url:contextPath+'/address/ajax/fourthCharts',
				type:'POST',
				dataType : 'json',	
				data:{},
				success:function(data){
					if(data.code == 1){
						oaAddress.service.dealChartsFour(data.attach);
					}
				},
				error:function(){
					//alert("操作失败");
		...
		setUserType : function(userName,userType,addReason){
			$.ajax({
				...
				type:'POST',
				dataType : 'json',	
				data:{"userName":userName,"userType":userType,"limitType":"address","addReason":addReason},
				success:function(data){
					if(data.code == 1){
						layer.msg("操作成功");
						oaAddress.service.refreshPage();
	        ...
	        solidgauge: {
	            ...
	            dataLabels: {
	                ...
	                formatter:function(){
	                	...
	                	return '<div>'+'<p style="color:#46517f;font-size:36px;font-weight: 600;">'+percent+'%'+'</p>'+'<br>'+''+'<p>'+'当天异常率'+'</p>'+'</div>';
	                }
	            },
	            linecap: 'round',

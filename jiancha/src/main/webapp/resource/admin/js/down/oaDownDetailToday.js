...
var downDeatilToday = {
		service : {
			init : function(){
				downDeatilToday.service.dealClick();
				downDeatilToday.controller.getPage(1);
			},
			dealClick : function(){
			    $(".data-table").on("click",".pageNumber", function(){
			    	var pageNo = $(this).attr("pageNo");
			    	downDeatilToday.controller.getPage(pageNo);
			    });
...
//			    	}
			    	...
			    	if(type=="black"){
			    		...
			    		window.location.href=contextPath+"/down/index"+params;
			    	}
			    });
			}
		},
		controller : {
			getPage : function(pageNo){
				...
				$.ajax({
					url:contextPath+'/down/index/todayDetailTable',
					type:'POST',
					dataType : 'html',	

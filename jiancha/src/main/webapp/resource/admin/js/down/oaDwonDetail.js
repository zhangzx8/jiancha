		...
		service : {
			...
			dealClick : function(){
				$("#fileDowWeekReturn").click(function(){
					window.location.href=contextPath+"/down/index/";
				});
		        $('.date').on('click', function(){
		        	...
		        	if($thisEle.is(':visible')) {
		            	...
		            	//var pageNo = $thisEle.parent().prev().val();
		...
		controller : {
			getChart : function(){
				...
				$.ajax({
					...
					type:'POST',
					dataType : 'json',	
					data:{"userName":userName,"busType":"down"},
			...
			getPage : function($thisEle, date, pageNo){
				...
				$.ajax({
					url:contextPath+'/down/index/detailTable',
					type:'POST',
					dataType : 'html',	
